"""Single imitation window preprocessing path shared by policy, replay and expert."""
...
class TemporalWindowPipeline:
    """Canonicalize and flatten chronological raw imitation windows.
    ...
    def _check_raw(self, windows: torch.Tensor) -> torch.Tensor:
        ...
        if windows.ndim != 3 or tuple(windows.shape[1:]) != expected:
            raise ValueError(
                f"imitation raw windows must have shape [B,{expected[0]},{expected[1]}], "
                f"got {tuple(windows.shape)}"
            )
        if windows.dtype != torch.float32:
            raise TypeError(f"imitation raw windows must be float32, got {windows.dtype}")
        if windows.numel() and not bool(torch.isfinite(windows).all()):
            raise ValueError("imitation raw windows contain non-finite values")
        return windows
    ...
    def normalize_flat(self, flat_windows: torch.Tensor, normalizer) -> torch.Tensor:
        if flat_windows.ndim != 2 or flat_windows.shape[1] != self.window_dim:
            raise ValueError(
                f"imitation flat windows must have shape [B,{self.window_dim}], "
