"""Frame-level trajectory replay with explicit continuity metadata."""
...
class FrameTrajectoryReplay:
    """CPU replay that stores each post-action imitation frame once.

    Windows are reconstructed only from predecessor chains that keep episode,
    environment, reference time and imitation age contiguous. Reset age-0 frames are
