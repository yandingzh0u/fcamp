...
The policy contribution is the coupling of primitive style rewards to causal
frame conditionals, using a shared prefix encoder with separate task/style Flow
value heads.
"""
...
class FCAMP(FlowCPSBase):
    ...
    def initial_reset(self) -> torch.Tensor:
        ...
        self._reset_imitation_history(self.env.phase_steps.clone())
        return obs
    ...
    def collect(self, current_obs: torch.Tensor) -> dict:
        ...
        # Rollout only stores legal policy windows. style rewards are recomputed
        ...
        imitation_window_chunks: list[torch.Tensor] = []
        imitation_window_index_chunks: list[torch.Tensor] = []
            ...
            for chunk_idx in range(chunks):
                ...
                for frame_idx in range(h):
                    ...
                    if bool(ready_mask.any()):
                        ...
                        imitation_window_chunks.append(raw_windows.detach().to("cpu", dtype=torch.float32))
                        imitation_window_index_chunks.append(flat_indices)
        ...
        rollout = {
            ...
            "imitation_windows": (
                torch.cat(imitation_window_chunks, dim=0)
                if imitation_window_chunks
                else torch.empty((0, self.imitation_window_dim), dtype=torch.float32)
            ),
            "imitation_window_indices": (
                torch.cat(imitation_window_index_chunks, dim=0)
                if imitation_window_index_chunks
    ...
    def _recompute_amp_rewards(self, rollout: dict) -> dict[str, float]:
        ...
        windows_cpu = rollout["imitation_windows"]
        indices_cpu = rollout["imitation_window_indices"]
        ...
        return {
            ...
            "amp/age0_in_reward_count": float(
                ((rollout["imitation_window_age"] == 0) & valid).sum().item()
            ),
        }
        ...
        *,
        ...
        for disc_step in range(update_steps):
            ...
            if replay_window_frames.shape[0] == 0:
                ...
                replay_raw = self.imitation_pipeline.flatten(replay_window_frames).to(
                    device=self.env.device,
                    dtype=torch.float32,
                    non_blocking=False,
                )
    ...
    def update(self, rollout: dict, collect_time: float) -> dict:
        ...
        if float(disc_metrics.get("disc/update_steps", 0.0)) <= 0.0:
            reward_metrics = {
                ...
                "amp/age0_in_reward_count": float(
                    ((rollout["imitation_window_age"] == 0) & rollout["amp_valid"]).sum().item()
                ),
