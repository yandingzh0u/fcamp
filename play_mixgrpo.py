...
def main() -> None:
    ...
    policy = FlowMatchingPolicy(
        ...
        time_embed_dim=train_cfg.get("time_embed_dim", 64),
        depth=train_cfg.get("depth", 4),
        action_limit=train_cfg.get("action_limit", 1.0),
    ).to(env.device)
    load_result = policy.load_state_dict(payload["policy"], strict=False)
    ...
    chunk_dim = train_cfg["horizon"] * train_cfg["action_dim"]
    ...
    while simulation_app.is_running():
        if cached_chunk is None or chunk_index >= train_cfg["horizon"]:
            noise = torch.zeros(env.num_envs, chunk_dim, device=env.device)

            with torch.inference_mode():
                cached_chunk = policy(current_obs, noise, steps=flow_steps)
            chunk_index = 0
