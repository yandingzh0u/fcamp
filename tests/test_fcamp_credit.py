...
def _credit(
    ...
    *,
    ...
    bootstrap = torch.ones(mask_shape) if bootstrap_mask is None else bootstrap_mask
    trace = torch.ones(mask_shape) if trace_mask is None else trace_mask
    valid = (
    ...
    return compute_dual_channel_gae(
        ...
        channel_valid_mask=valid.unsqueeze(-1).expand_as(rewards),
        channel_bootstrap_mask=bootstrap.unsqueeze(-1).expand_as(rewards),
        channel_trace_mask=trace.unsqueeze(-1).expand_as(rewards),
    )
...
def test_invalid_steps_are_zero_and_break_trace() -> None:
    ...
    torch.testing.assert_close(result.actor_advantage_components[1, 0], torch.zeros(2))
...
def test_actor_uses_one_weighted_global_advantage_normalizer() -> None:
    ...
    raw = compute_dual_channel_gae(
        ...
        channel_valid_mask=valid.unsqueeze(-1).expand_as(rewards),
        channel_bootstrap_mask=torch.ones_like(rewards),
...
def test_dual_flow_critic_shares_encoder_but_not_heads() -> None:
    ...
    assert all(
        ...
        for parameter in critic.amp_head.parameters()
    )
