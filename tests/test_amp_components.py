...
from amp.features import (
    AMPFeatureSchema,
    build_amp_frame,
    canonicalize_amp_window,
    quat_to_rot6d,
)
from amp.history import CausalAMPHistory
from amp.normalization import AMPRunningNormalizer
from amp.replay import AMPReplayBuffer
from amp.reward import amp_reward_from_logits
from networks.amp_discriminator import AMPDiscriminator, compute_amp_discriminator_loss


def test_feature_schema_is_233_and_policy_demo_share_builder() -> None:
    schema = AMPFeatureSchema()
    ...
    policy = build_amp_frame(**kwargs)
    demo = build_amp_frame(**kwargs)
...
def test_amp_window_canonicalization_uses_final_root_xy_and_preserves_height() -> None:
    ...
    canonical = canonicalize_amp_window(window)
    expected = raw.clone()
    expected[:, :, :2] -= raw[:, -1:, :2]
    ...
    torch.testing.assert_close(canonicalize_amp_window(shifted), canonical)


def test_history_order_push_and_subset_expert_reset() -> None:
    history = CausalAMPHistory(2, history_len=4, feature_dim=1)
    ...
    env1 = torch.tensor([1])
    history.reset(torch.tensor([[[20.0], [21.0], [22.0], [23.0]]]), env1)
...
def test_history_canonicalized_flatten_does_not_modify_raw_ring_frames() -> None:
    history = CausalAMPHistory(1, history_len=3, feature_dim=5)
    ...
    torch.testing.assert_close(canonical, canonicalize_amp_window(raw_window))
...
def test_history_rejects_uninitialized_push() -> None:
    history = CausalAMPHistory(1, history_len=4, feature_dim=2)
    with pytest.raises(RuntimeError, match="expert history"):
        history.push(torch.zeros(1, 2))


def test_amp_reward_matches_definition_and_clamps() -> None:
    logits = torch.tensor([-2.0, 0.0, 2.0, 100.0])
    rewards = amp_reward_from_logits(logits, scale=2.0)
...
def test_standard_discriminator_loss_has_bilateral_gp_and_backpropagates() -> None:
    torch.manual_seed(3)
    discriminator = AMPDiscriminator(8, hidden_dims=(16, 8))
...
def test_replay_wrap_dtype_sample_and_stats() -> None:
    replay = AMPReplayBuffer(5, 3, storage_dtype=torch.float16, pin_memory=False)
...
def test_normalizer_freeze_commit_and_state_dict_roundtrip() -> None:
    normalizer = AMPRunningNormalizer(2, clip=10.0)
    ...
    restored = AMPRunningNormalizer(2)
