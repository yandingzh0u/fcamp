...
def test_onpolicy_ratio_is_one():
    ...
    new, _, _ = a.get_cfm_loss(obs, action, eps, t)
    ratio = fpo.fpo_ratio(old.detach(), new, delta_clip=0.0)
...
def test_ratio_delta_clip_ste():
    ...
    ratio = fpo.fpo_ratio(old, new, delta_clip=2.0)
    ...
    unclamped = fpo.fpo_ratio(old, new.detach(), delta_clip=0.0)
