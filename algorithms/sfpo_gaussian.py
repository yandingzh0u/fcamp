...
"""SFPO: causal flow policy with action-Gaussian exploration.

This ablation keeps the original SFPO flow backbone, residual-absolute action
transform, per-frame PPO ratio, chunk GAE, and flow chunk-start critic. The only
exploration change is replacing in-flow path-space CPS with a plain diagonal
Gaussian over the executed action chunk.
"""
...
class SFPO(Algorithm):
    def build(self) -> None:
        ...
        if float(cfg.init_noise_std) <= 0.0:
            raise ValueError(f"init_noise_std must be > 0, got {cfg.init_noise_std}")
        self._policy.action_std = nn.Parameter(
            float(cfg.init_noise_std) * torch.ones(self.action_chunk_dim, device=env.device)
        )
        self.action_std = self._policy.action_std
    ...
    def _train_step_indices(self, device) -> torch.Tensor:
        # One stochastic density evaluation per executed frame; flow_steps still
        # controls the deterministic flow mean integration.
        return torch.arange(1, device=device, dtype=torch.long)
    ...
    def _flow_mean_latent(self, actor_obs: torch.Tensor) -> torch.Tensor:
        """Differentiable deterministic flow final latent used for eval/probing."""
        ...
        for step_index in range(steps):
            ...
            timestep_batch = torch.full((batch,), float(sigma.item()), device=latent.device, dtype=latent.dtype)
    ...
    def _action_std(self, *, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        return self.action_std.to(device=device, dtype=dtype).clamp(min=1.0e-4).view(1, self.horizon_h, self.num_act)

    def _action_log_prob(self, mean_actions: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        std = self._action_std(device=mean_actions.device, dtype=mean_actions.dtype)
        dist = Normal(mean_actions, mean_actions * 0.0 + std)
        return dist.log_prob(actions).sum(dim=-1)

    def _sample_action_gaussian(
        self,
        actor_obs: torch.Tensor,
        prev_action: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        mean_actions = self._flow_mean_actions(actor_obs, prev_action=prev_action)
        std = self._action_std(device=mean_actions.device, dtype=mean_actions.dtype)
        actions = mean_actions + torch.randn_like(mean_actions) * std
        log_probs = self._action_log_prob(mean_actions, actions)
    ...
    def collect(self, current_obs: torch.Tensor) -> dict:
        ...
        with torch.no_grad():
            for chunk_idx in range(chunks):
                ...
                action_chunk, step_log_probs, _ = self._sample_action_gaussian(actor_obs_n, prev_action)
    ...
    def update(self, rollout: dict, collect_time: float) -> dict:
        ...
        for epoch in range(int(self.cfg.policy_epochs)):
            ...
            for start in range(0, batch_size, mini_batch_size):
                ...
                for micro_start in range(0, mb_size, micro_batch_size):
                    ...
                    old_per_frame_logp = old_log_probs[sub]  # [micro, h]
                    mean_actions = self._flow_mean_actions(actor_obs[sub], prev_action=prev_action[sub])
                    new_per_frame_logp = self._action_log_prob(mean_actions, actions[sub])
                    per_frame_log_ratio = new_per_frame_logp - old_per_frame_logp  # [micro, h]
                    # Keep the same per-frame KL proxy used by SFPO.
        ...
        with torch.no_grad():
            ...
            action_std = self._action_std(device=device, dtype=torch.float32).reshape(-1)
        ...
        update_metrics = {
            ...
            "sfpo/action_gaussian_steps": float(train_step_indices.numel()),
            ...
            "policy/action_std_mean": float(action_std.mean().item()),
            "policy/action_std_min": float(action_std.min().item()),
            "policy/action_std_max": float(action_std.max().item()),
            "policy/action_std_params": float(self.action_std.numel()),
        }
        for k in range(h):
    ...
    def log(self, update_idx: int, max_updates: int, metrics: dict) -> None:
        ...
        print(
            ...
            f"density_steps={metrics.get('sfpo/action_gaussian_steps', float('nan')):.0f} "
            f"action_std={metrics.get('policy/action_std_mean', float('nan')):.4f} "
            f"[{metrics.get('policy/action_std_min', float('nan')):.4f},{metrics.get('policy/action_std_max', float('nan')):.4f}] "
            f"explore_params={metrics.get('policy/action_std_params', float('nan')):.0f}",
            flush=True,
        )
        print(
    ...
    def log_banner(self) -> None:
        cfg = self.cfg
        env = self.env
        print("[INFO] Starting SFPO training (flow mean + action Gaussian + chunk GAE)", flush=True)
        print(
            f"[INFO] task={env.task.name} terrain={env.task.terrain} motion_file={env.task.motion_file}",
            flush=True,
        )
        print(
            f"[INFO] algo=sfpo-gaussian actor_obs_dim={self.actor_obs_dim} critic_obs_dim={self.critic_obs_dim} "
            ...
            f"init_noise_std={cfg.init_noise_std}",
            flush=True,
        )
        print(
            ...
            f"exploration=action_diag_gaussian action_std_params={self.action_std.numel()} "
