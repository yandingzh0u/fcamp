...
def _log_validation_block(env, label: str, prefix: str, metrics: dict[str, float]) -> None:
    ...
    print(
        f"[{label}_CHUNK_RMS] "
        f"delta_boundary={metrics.get(f'{prefix}/chunk_action_delta_boundary_component_rms', float('nan')):.6f} "
        f"delta_internal={metrics.get(f'{prefix}/chunk_action_delta_internal_component_rms', float('nan')):.6f} "
        f"delta_ratio={metrics.get(f'{prefix}/chunk_action_delta_boundary_internal_component_rms_ratio', float('nan')):.4f} "
        f"d2_boundary={metrics.get(f'{prefix}/chunk_action_d2_boundary_component_rms', float('nan')):.6f} "
        f"d2_internal={metrics.get(f'{prefix}/chunk_action_d2_internal_component_rms', float('nan')):.6f} "
        f"d2_ratio={metrics.get(f'{prefix}/chunk_action_d2_boundary_internal_component_rms_ratio', float('nan')):.4f} "
        f"boundary_n={metrics.get(f'{prefix}/chunk_action_delta_boundary_component_count', 0.0):.0f} "
        f"internal_n={metrics.get(f'{prefix}/chunk_action_delta_internal_component_count', 0.0):.0f}",
        flush=True,
    )
