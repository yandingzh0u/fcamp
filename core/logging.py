"""Validation logging shared by all algorithms.

Per-update training diagnostics ([TRAIN]/[UPDATE_EFFECT]/[DONE]/[DONE_ROLLOUT]/[FIRST_FAILURE]/
[TRACK_*]/[ACT_SUMMARY]/...) are NOT shared: each algorithm owns a fully independent logging
implementation in its own module (ppo.py / fpo_pp.py / mixgrpo.py) so every algorithm only ever
prints fields it actually computes -- no cross-algorithm placeholder NaN/0. Validation, in
contrast, runs the exact same routine (core/validation.py) for every algorithm and produces an
identical metric dict, so its formatter legitimately stays here.
"""
