"""Memory-bounded CPU replay for policy AMP windows."""
...
class AMPReplayBuffer:
    """Raw chronological AMP windows stored in a CPU ring.
    ...
    def __init__(
        ...
        *,
        ...
        if storage_dtype not in (torch.float16, torch.float32, torch.bfloat16):
            raise ValueError("AMP replay storage must use a floating-point dtype")
