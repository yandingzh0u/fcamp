"""Pure-PyTorch construction of MimicKit-style AMP observations.
...
class AMPFeatureSchema:
    """Description of one AMP state and its causal history window."""
...
def canonicalize_amp_window(frames: torch.Tensor) -> torch.Tensor:
    """Apply MimicKit's G1 AMP root-translation convention to a window.
    ...
    if frames.ndim < 2:
        raise ValueError(f"AMP windows must have shape [..., W, F], got {tuple(frames.shape)}")
    if frames.shape[-2] <= 0:
        raise ValueError("AMP windows must contain at least one frame")
    if frames.shape[-1] < 3:
        raise ValueError("AMP frames must store root xyz in features 0:3")
...
def build_amp_frame(
    *,
    ...
    """Build one or more AMP frames with a common policy/demo code path.
    ...
    positions are root-relative by default, matching MimicKit's G1 AMP feature.
    """
    ...
    if mismatched:
        raise ValueError(f"AMP feature leading dimensions must match {leading}; got {mismatched}")
    ...
    if frame.shape[-1] != schema.frame_dim:
        raise RuntimeError(f"internal AMP feature size error: {frame.shape[-1]} != {schema.frame_dim}")
    return frame
