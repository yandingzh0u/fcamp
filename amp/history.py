"""Per-environment causal AMP history with explicit episode age."""
...
class CausalAMPHistory:
    ...
    def __init__(
        ...
        *,
        ...
        if dtype != torch.float32:
            raise ValueError("AMP history must store lossless float32 frames")
    ...
    def push(self, frame: torch.Tensor, env_ids: torch.Tensor | None = None) -> None:
        ...
        if not bool(self._initialized[ids].all()):
            ...
            raise RuntimeError(f"AMP history must be reset before push; uninitialized={bad}")
    ...
    def window_ages(self, env_ids: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if not bool(self.ready[ids].all()):
            ...
            raise RuntimeError(f"AMP history not ready: env_ids={bad}, ages={ages}")
        ...
        if not torch.equal(slot_ages, expected) or bool((slot_ages <= 0).any()):
            raise RuntimeError("corrupt AMP history: legal window ages must be positive and contiguous")
        return slot_ages
